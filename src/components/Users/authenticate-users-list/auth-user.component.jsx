...
const AuthUser = ({ user, handleAccept, handleReject }) => {
    ...
    return (
        <tr className="bg-[#f0f0f0] border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200 hover:bg-[#c8c8c8] dark:hover:bg-gray-600">
            ...
            <td className="px-6 py-4 space-x-4">
                <button onClick={() => handleAccept(user)} className="bg-[#1b6c64] text-[#e0e1db] text-sm w-24 h-10 rounded-lg active:scale-95 transition ease-in-out duration-300 text-center">ACCEPT</button>
                <button onClick={() => handleReject(user)} className="bg-red-600 text-[#e0e1db] text-sm w-24 h-10 rounded-lg active:scale-95 transition ease-in-out duration-300 text-center">REJECT</button>                               
            </td>
        </tr>
    );
}
